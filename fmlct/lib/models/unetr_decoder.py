...
class UNETR_decoder(nn.Module):
    ...
    def forward(self, x_in, x, hidden_states_out):
        ...
        enc1 = self.encoder1(x_in)
        #print("enc1 size:", enc1.size())  # 打印 enc1 的尺寸

        x2 = hidden_states_out[3]
        #print("x2:", x2.size())
        enc2 = self.encoder2(self.proj_feat(x2, self.hidden_size, self.grid_size))
        #print("enc2 size:", enc2.size())  # 打印 enc2 的尺寸

        x3 = hidden_states_out[6]
        #print("x3:", x3.size())
        enc3 = self.encoder3(self.proj_feat(x3, self.hidden_size, self.grid_size))
        #print("enc3 size:", enc3.size())  # 打印 enc3 的尺寸

        # x4 = hidden_states_out[9]
        # print("x4:", x4.size())
        # enc4 = self.encoder4(self.proj_feat(x4, self.hidden_size, self.grid_size))
        # print("enc4 size:", enc4.size())  # 打印 enc4 的尺寸

        dec3 = self.proj_feat(x, self.hidden_size, self.grid_size)
        #print("dec3 size:", dec3.size())  # 打印 dec4 的尺寸

        dec2 = self.decoder4(dec3, enc3)
        #print("dec2 size:", dec2.size())  # 打印 dec3 的尺寸

        dec1 = self.decoder3(dec2, enc2)
        #print("dec1 size:", dec1.size())  # 打印 dec2 的尺寸

        # dec1 = self.decoder3(dec2, enc2)
        # print("dec1 size:", dec1.size())  # 打印 dec1 的尺寸

        out = self.decoder2(dec1, enc1)
        #print("out size before UnetOutBlock:", out.size())  # 打印最终输出前的尺寸
